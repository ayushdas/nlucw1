...
class RNN(object):
	...
	def predict(self, x):
		...
		for t in range(len(x)):
			##########################
			if (t == 0):
				netin = self.V.dot(make_onehot(x[t],self.vocab_size)) + self.U.dot(s[len(x)])
			else:
				netin = self.V.dot(make_onehot(x[t],self.vocab_size)) + self.U.dot(s[t-1])
			sigmoid_v = np.vectorize(sigmoid) # denoted by f()
			s[t] = sigmoid_v(netin)
			netout = self.W.dot(s[t])
			y[t] = softmax(netout) # denoted by g()
			##########################
	...
	def acc_deltas(self, x, d, y, s):
		...
		for t in reversed(range(len(x))):
			##########################
			gprime_net_out = np.ones(len(x))	
			delta_out = (make_onehot(d[t], self.vocab_size)-y[t]) * gprime_net_out[t]
			deltaW_sum += np.outer(delta_out, s[t])
			fprime_net_in = s[t] * (np.ones(len(s[t])) - s[t])
			delta_in = (np.dot(self.W.T, delta_out)) * fprime_net_in
			deltaV_sum += np.outer(delta_in, make_onehot(x[t], self.vocab_size))
			deltaU_sum += np.outer(delta_in, s[t-1])
			##########################	
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		for t in reversed(range(len(x))):
			...
			##########################
			delta_out = (make_onehot(d[t], self.out_vocab_size) - y[t]) * 1
			deltaW_sum += np.outer(delta_out, s[t])
	...
	def compute_mean_loss(self, X, D):
		...
		for i in range(len(D)):
			words_in_d += len(D[i])
		for t in range(len(X)):
			loss += self.compute_loss(X[t],D[t])
		mean_loss = loss / words_in_d	
		##########################
